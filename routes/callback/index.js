...
const { verifyIpn } = require("@bananastressreliever/skinsdrip-sdk");
...
module.exports = (io) => {
  // @desc    Get oxapay ipns
  // @route   POST /callback/oxapay
  // @access  Public
      ...
      try {
        ...
        let dataDatabase = await Promise.all([
          CryptoAddress.findOne({ name: currency, address: req.body.address })
            ...
            .lean(),
          CryptoTransaction.findOne({ "data.providerId": transactionId })
        ...
        if (
          ...
          if (dataDatabase[0].user.affiliates.referrer !== undefined) {
            promises.push(
              User.findByIdAndUpdate(
                dataDatabase[0].user.affiliates.referrer,
                {
                  $inc: {
          ...
          io.of("/cashier")
            .to(dataDatabase[1].user.toString())
            .emit("cryptoTransaction", { transaction: dataDatabase[1] });
        }

        // Remove transaction id from crypto block array
        callbackBlockTransactionCrypto.splice(
          callbackBlockTransactionCrypto.indexOf(transactionId.toString()),
          1
        );
  ...
  // @desc    Get skinsdrip ipns
  // @route   POST /callback/skinsdrip
  // @access  Public
  router.post("/skinsdrip", rateLimiterMiddleware, async (req, res) => {
    try {
      ...
      if (!isValidIpn) {
        return res
          .status(400)
          .json({ success: false, error: "Invalid Ipn call" });
      }
      ...
      // Validate crypto transaction
      if (callbackBlockTransactionCredit.includes(orderId.toString())) {
        return res
          .status(400)
          .json({ success: false, error: "Transaction already processed" });
      }
      ...
      try {
        // Add transactions id to crypto block array
        callbackBlockTransactionCredit.push(orderId.toString());

        const transactionDatabase = CreditTransaction.findOne({
        ...
        if (
          ...
          // req.body.status === "Paid" &&
          transactionDatabase &&
          value === transactionDatabase.amount &&
          transactionDatabase.state === "created"
        ) {
          // Create promises array
          let promises = [];

          // Get transaction amount in fiat (assuming payAmount is in the smallest unit of the cryptocurrency)
          const amountFiat = Math.floor((value / 100) * 1000);
        ...
        // Remove transaction id from crypto block array
        callbackBlockTransactionCredit.splice(
          callbackBlockTransactionCredit.indexOf(orderId.toString()),
          1
        );
        ...
        console.log(err);
        callbackBlockTransactionCredit.splice(
          callbackBlockTransactionCredit.indexOf(transactionId.toString()),
          1
        );
        res.status(500).json({
          success: false,
  ...
  router.post("/skinify", rateLimiterMiddleware, async (req, res) => {
    try {
      ...
      const token = crypto
        .createHash("md5")
        .update(process.env.SKINIFY_API_KEY)
        .digest("hex");
      ...
      try {
        ...
        const transactionDatabase = await SteamTransaction.findOne({
          "data.providerId": transactionId,
        })
        ...
        if (
          ...
          if (req.body.status === "success") {
            ...
            promises = [
              SteamTransaction.findByIdAndUpdate(
                transactionDatabase._id,
                {
                  amount: amount,
                  state: "completed",
        ...
        callbackBlockTransactionSteam.splice(
          callbackBlockTransactionSteam.indexOf(transactionId.toString()),
          1
        );
  ...
  //       transactionId,
  //       callbackBlockTransactionCredit
  //     );
